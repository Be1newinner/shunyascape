...
import { BuildType, CityStats, GridCell, HumanAgent, VehicleAgent, AnimalAgent, Particle, SimContext } from './Types';
...
import { createHouseMesh, createSkyscraperMesh, createConstructionSiteMesh } from './Home';
import { WeatherManager } from './Weather';
import { CameraManager } from './Camera';
import { spawnAnimals, updateAnimals } from './NPCAnimals';
import { spawnVehicleOnRoad, cleanStrandedVehicles, updateVehicles } from './Vehicle';
import {
...
export class ThreeCity {
  ...
  // Caching Methods
  public getGeometry = (name: string, creator: () => THREE.BufferGeometry): THREE.BufferGeometry => {
  ...
  private getSimContext(): SimContext {
    return {
      ...
      spawnVehicleOnRoad: (x: number, z: number) => {
        spawnVehicleOnRoad(this.getSimContext(), this.vehicles, this.humans, x, z);
      }
    };
  }
  ...
  private onPointerMove = (event: PointerEvent) => {
    this.cameraManager.rotateCameraOnHover(event, !!this.player, this.buildMode, this.isAdmin);
    ...
    if (intersects.length > 0 && this.buildMode && this.isAdmin) {
      ...
      if (gx >= 0 && gx < this.gridSize && gz >= 0 && gz < this.gridSize) {
        ...
        if (this.buildMode === 'delete') {
          (this.buildPreview.material as THREE.MeshBasicMaterial).color.setHex(0xff0000);
        } else {
          const isValid = cell.type === 'empty';
          (this.buildPreview.material as THREE.MeshBasicMaterial).color.setHex(isValid ? 0x00ff00 : 0xffa500);
        }
        return;
      }
    }
  ...
  private onPointerDown = (event: PointerEvent) => {
    if (event.button !== 0) {
      if (event.button === 2) {
        event.preventDefault(); // Prevent browser context-menu gesture from canceling pointer capture
      }
      return;
    }
  ...
  private orderConstruction(x: number, z: number, type: 'road' | 'tree' | 'house' | 'skyscraper') {
    ...
    const constructionMesh = createConstructionSiteMesh(this.getSimContext(), x, z);
  ...
  private demolishCell(x: number, z: number) {
    ...
    if (oldType === 'road') {
      this.recalculateRoadConnections();
      this.vehicles = cleanStrandedVehicles(this.getSimContext(), this.vehicles, this.grid);
    }
  ...
  private spawnHumanAtRandomHouse() {
    ...
    const mesh = createRefinedHumanMesh(this.getSimContext(), clothingColor, false, agent);
    humanGroup.add(mesh);
  ...
  public loadAllDatabaseUsers(users: any[], currentPlayerEmail: string) {
    if (this.isDestroyed) return;
    loadAllDatabaseUsers(this.getSimContext(), this.humans, users, currentPlayerEmail);
  }
      ...
      if (needsUpdate) {
        ...
        if (c.type !== 'empty') {
          if (c.type === 'construction') {
            localCell.mesh = createConstructionSiteMesh(this.getSimContext(), c.x, c.z);
          } else {
  ...
  private spawnNpcFromData(n: any) {
    ...
    const agent: Partial<HumanAgent> = {};
    const mesh = createRefinedHumanMesh(this.getSimContext(), n.clothingColor, false, agent);
    humanGroup.add(mesh);
  ...
  private animate = () => {
    ...
    // 1. Weather Update
    this.timeOfDay = this.weatherManager.updateTime(this.timeOfDay, this.timeSpeed, delta);
    ...
    updateHumans(this.getSimContext(), this.humans, this.keysPressed, delta, syncStates);
  ...
  public isPlayerInsideBlockedCell(): boolean {
    ...
    const cell = this.grid[gx][gz];
    return cell.type === 'house' || cell.type === 'skyscraper' || cell.type === 'tree' || cell.type === 'construction';
  }
  ...
  public teleportPlayerToSafeCell() {
    ...
    if (foundCell) {
      const targetWorldX = (foundCell.x * this.cellSize) - halfGrid + this.cellSize / 2;
      const targetWorldZ = (foundCell.z * this.cellSize) - halfGrid + this.cellSize / 2;
